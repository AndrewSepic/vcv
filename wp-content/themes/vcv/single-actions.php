<? get_header(); ?>
    	...
    	<div class="container" id='searchContainer'>
	    	<section id="search" class='clearfix'>
				...
				<form method='get' action='/search' id="searchForm">
					...
					<a href='' id='submit'><img src='<? echo  get_template_directory_uri();  ?>/img/formSubmit.png' /></a>
				</form>
			</section>
    ...
    <section id='contacts'  class='container clearfix <?  echo $activeContacts;  ?> '>
    	<?
    	$modes=get_field("contacts");
    	?>
    	<nav> 
    		<ul>
    			<? if(in_array("Facebook", $modes)): ?>
    			<li><a href='facebook_text' class='facebook' >Facebook</a></li>
    			<? endif; ?>
    			<? if(in_array("Phone", $modes)): ?>
    	...
    	<div id='intro_text' class='info'>
    		<h2>Contact <span class='contactName'><?  the_field('contact_name'); ?></span></h2>
    	</div>
		<? endif; ?>
    	
    	<? if(in_array("Facebook", $modes)): ?>
    	<div id='facebook_text' class='info'>
    		<h2>Contact <span class='contactName'><?  the_field('contact_name'); ?></span> on Facebook</h2>
    		<? $fbLinkText=str_replace("https://","",get_field('facebook_address')); ?>
    		 <p><a href="<? the_field('facebook_address'); ?>" target='blank'  class='featureLink fblink'><? echo $fbLinkText; ?></a></p>
    		<?  the_field('facebook_text'); ?>
    	</div>
    	<? endif; ?>
    	<? if(in_array("Phone", $modes)): ?>
    	<div id='phone_text' class='info'>
    		<h2>Call <span class='contactName'><?  the_field('contact_name'); ?></span></h2>
    		<p class='featureLink' >CALL: <a href='tel:<?  the_field('phone_number'); ?>' class='featureLink phoneNumber' ><?  the_field('phone_number'); ?></a></p>
    		<?  the_field('phone_number_text'); ?>
    	</div>
    	<? endif; ?>
    	<? if(in_array("Email", $modes)): ?>
    	<div id='email_text' class='info'>
    		<h2>Contact <span class='contactName'><?  the_field('contact_name'); ?></span> by Email</h2>
    		<?  the_field('email_text'); ?>
    		<p  class='featureLink'>EMAIL: <a href="mailto:<? the_field('email_address'); ?>" class='featureLink emailLinkA'><? the_field('email_address'); ?></a></p>
    		<form id="takeActionForm">
	    		...
	    		<input type="text" id="subject"   value="<? the_field('email_subject'); ?>" />
    			<label>Please personalize the message below:</label>
    			<textarea id="message"><? the_field('email_message'); ?></textarea>   			
    		</form>
    		<nav id="emailMenu">

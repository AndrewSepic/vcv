<? get_header(); ?>
<script src="<? echo get_template_directory_uri(); ?>/js/tablesorter-master/jquery.tablesorter.min.js"></script>
				...
				<div class='copy'>
					...
					<? if ($chamber=="Senate"): ?>
						...
						if( get_field('second_party')){ ?>
							<span class='party second'><? echo substr(get_field('second_party'),0,1); ?></span>
						<? }
						 ?>
					...
					<p>
						...
						<? }
						 ?>
						<span class='towns'><? echo get_district(get_the_id()); ?></span>
					</p>
					<p>
						<span class='towns'><? the_field('towns'); ?></span>
					</p>
					...
					?>
					<p id="scores"><span><? echo $year; ?>  Score: <strong><? echo $currentYear; ?></strong> </span><span>Lifetime Score:  <strong><? echo $lifetime ?>%</strong> </span></p>
					</div>
				</section>	
       ...
       <? /* bills */	   	   $terms = get_terms( array( 'taxonomy' => 'scorecard', 'hide_empty' => true) );	   	   $years=array();	   foreach($terms as $t ){		   $y= $t->name;		   array_push($years, $y);	   }	   	   rsort($years,SORT_NUMERIC);	   	   echo "<nav class='tabs'>";	   foreach($years as $y ){			$active='';			if($y==$year){			   $active='active';		   }		   		   echo "<a href='' data-year='$y' class='tab $active'>".$y." Bills</a>";	   }	   echo "</nav>";	   		foreach($terms as $t ){			$y= $t->name;						 getSingleBills(get_the_id(), $y, $year);;		}		  	   
        ?>
     			<div id="key" class='clearfix'>
     				...
     				<div>
     				<img src='<? echo get_template_directory_uri(); ?>/img/thumbs_down_big.gif' />
     				Anti-Environment Vote
     				</div>
     				
     				<div>
     				<img src='<? echo get_template_directory_uri(); ?>/img/NIO.gif' />
     				Not in Office
     				</div>
     				
     				<div>
     				<img src='<? echo get_template_directory_uri(); ?>/img/Absent.gif' />
     				Absent
     				</div>
     				
     				<div>
     				<img src='<? echo get_template_directory_uri(); ?>/img/Presiding.gif' />
     				Presiding
     				</div>
     				
     				<div>
     				<img src='<? echo get_template_directory_uri(); ?>/img/Abstain.gif' />
     				Abstain
     				</div>
     				<div>
     				<img src='<? echo get_template_directory_uri(); ?>/img/Suspended.gif' />
     				Suspended
     				</div>
